...
The company to run it. Provide unit tests. At minimum, you must provide a unit test capable of consuming a 
...
criteria given to you in `CSV`. Furthermore, the company will test your program with several additional 
inputs that you are not given. Ideally, we would like to have a unit test which reads some 
directory (eg: `/tmp/company`) and validates your program against any number of `CSV` files inside 
